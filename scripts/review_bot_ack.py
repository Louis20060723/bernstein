...
# OUT of this set when their GitHub App is removed: a login listed here is a
# login the coverage section reports as absent on every PR, which reads as a
# permanent gap rather than a retirement.
REVIEW_BOT_LOGINS = {"baz-reviewer[bot]"}

# Tags that mark a finding as must-address. Matching is case-insensitive
# against the comment body. The patterns cover the common severity headings
# review bots embed, e.g. `**Potential issue**`, `_⚠️ Potential issue_`,
...
# Review bots reply with a quota notice - as a submitted review or a top-level
# comment - when they decline to review a diff; that notice must not count as
# review coverage.
DID_NOT_RUN_PATTERNS = (
    r"review limit reached",
    r"reached your (?:daily|weekly|monthly) rate limit",
)
...
class BotArtifact:
    ...
    is ``None`` for top-level issue comments, which is how some bots post
    their review; that shape is anchored via the head SHA in ``body`` instead.
    """
...
def covers_head(artifact: BotArtifact, head_sha: str) -> bool:
    ...
    names - a bot's summary comment states the range it reviewed.
    """
    if not head_sha:
        return False
...
def findings_from(sources: dict[str, list[Any]]) -> list[Finding]:
    ...
    for c in sources.get("issue-comment", []):
        ...
        if login not in REVIEW_BOT_LOGINS:
            continue
        body = c.get("body") or ""
        # Skip quota notices; they're coverage signals, not findings.
        if looks_like_a_declined_review(body):
            continue
        sev = classify(body)
